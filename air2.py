...
class AirData:
  ...
  def analysis(self,para):
    ...
    train_x = train_data.drop(columns=['Class', 'Temperature', 'Humidity'],axis=1)#, 'Temperature', 'Humidity' version2 needs to drop "Temp,Humidity cols"
    ...
    if self.trainning:
        ...
        # model.fit(train_x,train_y, eval_metric="error", eval_set=eval_set, verbose=True)
    ...
    if self.csv:
        ...
        # seperate the independent and target variable on training data
        test_data_x = test_data.drop(columns=['Class', 'Temperature', 'Humidity'],axis=1)
        ...
        test_x=np.array([[0,float(e0.get()),float(e1.get()),float(e2.get()),float(e3.get())]])
        df_test = pd.DataFrame(data=test_x, index=[0], columns=["ID", "TGS 826", "TGS 822", "TGS 2600", "TGS 2602"])
        print(df_test)
        predict_test = model.predict(df_test)
        #print('\nTarget on test data',predict_test)
        
        ###### Conf, Prob Determinations for range mapping-determinations
        ...
        output='Target on test data(Range): '+range_res #predict_test(Both can be kept(Logs,Range) too. now kept as Log-Range)
        # label = Label(root, text=output)
        # label.pack()
        output+="\n [Logs]: "
        textWid.insert(END, output)
